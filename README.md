...
- 🔑 **API Key 管理**：安全輸入 Gemini API Key，可直接點擊按鈕前往申請
- 📊 **靈活選擇**：支援 1、8、16、24、32、40 張貼圖生成（預設 1 張，適合測試 API Key 和單張重生成）
...
1. **填入 Gemini API Key**（可點擊「申請 API Key」按鈕前往 Google AI Studio 申請）
2. **選擇創作張數**（1、8、16、24、32 或 40 張，預設為 1 張）
3. **填入主題描述或上傳角色圖片**
...
1. 在工具中點擊「申請 API Key」按鈕，或直接前往 [Google AI Studio](https://aistudio.google.com/)
2. 登入您的 Google 帳號
3. 創建新的 API Key
4. 複製 API Key 備用
...
按照步驟提示操作：
1. 輸入 Gemini API Key（可點擊「申請 API Key」按鈕前往申請）
2. 選擇要生成的貼圖張數（預設為 1 張，適合測試）
3. 輸入主題描述或上傳角色圖片
4. 生成並確認角色
5. 生成並編輯文字描述
6. 生成 8 宮格貼圖
...
- **1 張**：適合測試 API Key 是否有效、免費額度是否足夠，或僅需重新生成單張貼圖時使用
- **8、16、24、32、40 張**：正式製作 LINE 貼圖時使用（LINE 官方要求至少 8 張）
